...
APP_BRANCH=${1:-main}
BASE_URL=$(ip -4 -br addr show wlan0 | awk '{print $3}' | cut -d/ -f1):3000
...
docker exec -u weasyprint vanilla /bin/bash -c "BASE_URL='$BASE_URL' /opt/weasyprint/app/.venv/bin/uwsgi --ini /etc/opt/ds/weasyprint.ini"
